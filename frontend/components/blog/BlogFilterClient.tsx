...
export default function BlogFilterClient({ allPosts, allCategories }: BlogFilterClientProps) {
  ...
  console.log('[FILTER DEBUG] Received allPosts length:', allPosts.length);
  console.log('[FILTER DEBUG] Received allCategories:', allCategories);

  const filterPostsByCategory = useCallback((posts: BlogPost[], category: string | null): BlogPost[] => {
    if (!category) return posts;
    return posts.filter(post => post.category === category);
  }, []);

  const filteredPosts = filterPostsByCategory(allPosts, currentCategory);
  console.log('[FILTER DEBUG] currentCategory:', currentCategory, 'filteredPosts length:', filteredPosts.length);
